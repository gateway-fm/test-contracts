...
import { bytesStruct, intStruct, mixedStruct, uintStruct } from "../../test/testData";

export async function emitIntEvents(hre: HardhatRuntimeEnvironment) {
  const contract = await getTestValueEmitterContract(hre);

  try {
    await contract.emitIntEvents(intStruct).then(tx => waitForTxAndPrintHash(tx));
  } catch (e) {
...
export async function emitUintEvents(hre: HardhatRuntimeEnvironment) {
  ...
  try {
    await contract.emitUintEvents(uintStruct).then(tx => waitForTxAndPrintHash(tx));
  } catch (e) {
    const err = e as Error;
...
export async function emitBytesEvents(hre: HardhatRuntimeEnvironment) {
  ...
  try {
    await contract.emitBytesEvents(bytesStruct).then(tx => waitForTxAndPrintHash(tx));
  } catch (e) {
    const err = e as Error;
...
export async function emitMixedEvents(hre: HardhatRuntimeEnvironment) {
  ...
  try {
    await contract.emitMixedEvent(mixedStruct).then(tx => waitForTxAndPrintHash(tx));
  } catch (e) {
    const err = e as Error;
